...
const get = async (req: NextApiRequest, res: NextApiResponse) => {
  ...
  const albumExist =
    allAlbum.find(({ text }) => text.toLowerCase() === album?.toLowerCase())?.href ??
    allAlbum.find(({ text }) => text.toLowerCase().includes(album.toLowerCase()))?.href;

  if (!albumExist) {
    return res.status(404).json({ message: 'no album review' });
  }

  const albumPage = await fetch(`https://pitchfork.com${albumExist}`).then((r) => r.text());
